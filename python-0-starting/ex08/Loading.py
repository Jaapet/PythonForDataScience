...
def ft_tqdm(lst: range) -> None:
    ...
    takes an iterable and displays a loading bar in the terminal while
    iterating through the provided range. It shows the progress percentage,
    a visual bar, elapsed time, estimated remaining time,
    and iterations per second.

    Parameters:
    ----------
    lst : range
        An iterable range to iterate through.
        The function will process each item in the range,
    ...
    for i, value in enumerate(lst):
        ...
        print(f"\r{int(p):3d}%|{loading_bar}| {i}/{length}\
